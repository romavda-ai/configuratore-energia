...
(function(){
  ...
  async function generate(data){
    ...
    // Small table (placeholder: in futuro si può passare data dettagliata)
    doc.setDrawColor(220,220,220);
    doc.setFillColor(247,247,247);
    doc.rect(M, y, W-2*M, 8, "FD");
    pickFont(doc,true); doc.setFontSize(9); doc.setTextColor(...colGray);
    doc.text("Periodo", M+2, y+5.5);
    doc.text("kWh Tot.", M+40, y+5.5);
    doc.text("Spesa attuale", M+62, y+5.5);
    doc.text("Spesa Fastweb", M+95, y+5.5);
    doc.text("Costo medio €/kWh", M+130, y+5.5);

    y += 8;
    // row 1
    doc.setFillColor(255,255,255);
    doc.rect(M, y, W-2*M, 8, "FD");
    pickFont(doc,false); doc.setFontSize(9); doc.setTextColor(...colBlack);
    const period1 = safe(data.mese1) + (data.anno? ("/"+safe(data.anno)) : "");
    const kwh1 = (safe(data.tipologia).toUpperCase().includes("MULTI"))
      ? (data.kwh1?.f1 + data.kwh1?.f2 + data.kwh1?.f3)
      : (data.kwh1?.f1 || 0);
    doc.text(period1 || "-", M+2, y+5.5);
    doc.text(String(Math.round(kwh1||0)), M+40, y+5.5);
    doc.text(fmtEUR(att1), M+62, y+5.5);
    doc.text(fmtEUR(fw1), M+95, y+5.5);
    const cm = (kwh1>0) ? (fw1/kwh1) : 0;
    doc.text(cm ? cm.toFixed(5) : "-", M+130, y+5.5);

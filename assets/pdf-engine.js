...
// Detail table
  ...
  // Fee (quota fissa fastweb): 5 se SI, 20 se NO. Moltiplicata per mesi (mensilità=2 → x2).
  const vodSI = String(data?.clienteVodafone ?? "").trim().toUpperCase() === "SI";
  const feeMonth = vodSI ? 5 : 20;
  const feeTot = (String(mensilita).trim()==="2") ? (feeMonth*2) : feeMonth;

  // Valori attuali (quota consumo / fissa)
  const attCons = (String(mensilita).trim()==="2") ? (normNum(txt("bimQuotaConsumi")) || (data?.spesaBim?.quotaConsumi ?? 0)) : (normNum(txt("spesaAtt1")) || (data?.spesaAtt1?.consumo ?? 0));
  const attFix  = (String(mensilita).trim()==="2") ? (normNum(txt("bimQuotaFissa"))   || (data?.spesaBim?.quotaFissa   ?? 0)) : (normNum(txt("spesaAtt1Fissa")) || (data?.spesaAtt1?.fissa   ?? 0));
  ...
  const cols = [
    {h: safe(pdfT.thPeriodo || "Periodo"), w: 0.20},
    {h: safe(pdfT.thKwh || "kWh Tot."), w: 0.12},
    {h: safe(pdfT.thAttCons || "Spesa Consumi"), w: 0.16},
    {h: safe(pdfT.thAttFix || "Spesa Fissa"), w: 0.14},
    {h: safe(pdfT.thFwCons || "Quota Consumi Fastweb"), w: 0.24},
    {h: safe(pdfT.thFwFix || "Quota Fissa Fastweb"), w: 0.14},
  ];
  ...
  const period1 = safe(periodo1 || (mese1 + (anno ? ("/"+anno) : "")));
  const kwhTot = (String(tipologia).toUpperCase().includes("MULTI"))
    ? (kwhF1_1 + kwhF2_1 + kwhF3_1)
    : (kwhMono_1);

  const row = [
    period1 || "-",
    String(Math.round(kwhTot || 0)),
    fmtMoney(attCons),
    fmtMoney(attFix),
    fmtMoney(fwCons),
    fmtMoney(fwFix),

/* pdf-engine.js (v56) - motore PDF self-contained
   Dipendenze: jsPDF UMD già caricato (assets/jspdf.umd.min.js). Se non presente tenta CDN.
   API: window.PDFEngine.generateFromDOM()
*/
(function(){
  const safe = (s)=> String(s ?? "").trim();
  const $ = (id)=> document.getElementById(id);
  const val = (id)=> safe($(id)?.value);
  const text = (id)=> safe($(id)?.textContent);
  ...
  async function ensureJsPdfLoaded(){
    if(window.jspdf && (window.jspdf.jsPDF || window.jspdf.default) ) return true;
    const candidates = [
  ...
  function readDataFromDOM(){
    const mensilita = val("mensilita") || "1";
    const tipologia = val("tipologia") || val("tipoContatore") || "";
    const offerta = val("offerta") || val("offertaEconomica") || "";
    const clienteVodafone = (val("clienteVodafone") || val("vodafone") || "").toUpperCase();

    const mese1 = val("mese1") || val("mese") || "";
    const anno  = val("anno") || "";

    const kwh1 = {
      f1: normNum(val("kwh1_f1") || val("kwh_f1")),
      f2: normNum(val("kwh1_f2")),
      f3: normNum(val("kwh1_f3")),
    };

    // Spesa attuale 1 mese (consumi + fissa)
    const attCons1 = normNum(val("spesaAtt1"));
    const attFix1  = normNum(val("spesaAtt1Fissa"));

    // Bimestre (modal)
    const bimCons = normNum(val("bimQuotaConsumi"));
    const bimFix  = normNum(val("bimQuotaFissa"));

    // Totali Fastweb già calcolati dal configuratore
    const fw1   = normNum(val("out_fw1")) || normNum(text("out_fw1"));
    const fwBim = normNum(val("out_fw_bim")) || normNum(text("out_fw_bim"));

    // Totali attuale
    const att1Tot = (String(mensilita).trim()==="1") ? (attCons1 + attFix1) : attCons1;
    const attBimTot = (bimCons + bimFix) || normNum(val("out_att_bim")) || normNum(text("out_att_bim"));

    const cliente = {
      ragSoc: val("ragSoc") || val("ragione_sociale") || "",
      piva:   val("piva")   || val("vat") || "",
      pod:    val("pod")    || val("idpod") || "",
      indirizzo: val("indirizzo") || val("indirizzoPod") || "",
    };

    // Riferimenti (se gestiti)
    const agent = {
      name:  (window.PDFCFG?.style?.agentName ?? "Stefano Santarelli"),
      email: (window.PDFCFG?.style?.agentEmail ?? "stefano.santarelli@wincomitalia.com"),
      company: (window.PDFCFG?.style?.agentCompany ?? "Wincom Srl"),
      partner: (window.PDFCFG?.style?.agentPartner ?? "Vodafone Excellent Partner"),
    };

    return {
      mensilita, tipologia, offerta, clienteVodafone, mese1, anno,
      kwh1,
      att: { cons1: attCons1, fix1: attFix1, tot1: att1Tot, bimCons, bimFix, totBim: attBimTot },
      fw: { m1: fw1, bim: fwBim },
      cliente, agent
    };
  }

  async function generateFromDOM(){
  ...
  async function generate(d){
    ...
    const jsPDFCtor = (window.jspdf && window.jspdf.jsPDF) ? window.jspdf.jsPDF : (window.jspdf?.default || null);
    if(!jsPDFCtor) throw new Error("jsPDF ctor non trovato");

    const mensilita = safe(d.mensilita || "1");
    const periodoTxt = (mensilita==="2") ? "bimestre" : "mensile";

    const attTot = (mensilita==="2") ? (d.att.totBim || 0) : (d.att.tot1 || 0);
    const fwTot  = (mensilita==="2") ? (d.fw.bim || 0)     : (d.fw.m1 || 0);
    ...
    const vodSI = safe(d.clienteVodafone).toUpperCase()==="SI";
    const feeMonth = vodSI ? 5 : 20;
    const feeTot = (mensilita==="2") ? (feeMonth*2) : feeMonth;

    const attCons = (mensilita==="2") ? (d.att.bimCons || 0) : (d.att.cons1 || 0);
    const attFix  = (mensilita==="2") ? (d.att.bimFix  || 0) : (d.att.fix1  || 0);
    ...
    const cl = d.cliente || {};
    const pr = {
      offerta: safe(d.offerta),
      vodafone: safe(d.clienteVodafone),
      periodo: safe(d.mese1) + (d.anno? ("/"+safe(d.anno)) : ""),
      tipo: safe(d.tipologia)
    };
    ...
    const period1 = safe(d.mese1) + (d.anno? ("/"+safe(d.anno)) : "");
    const kwhTot = safe(d.tipologia).toUpperCase().includes("MULTI")
      ? ((d.kwh1.f1||0) + (d.kwh1.f2||0) + (d.kwh1.f3||0))
      : (d.kwh1.f1||0);

    doc.text(period1 || "-", xP, y+5.5);
    doc.text(String(Math.round(kwhTot||0)), xK, y+5.5);
    ...
    // Riferimenti commerciali (riquadro grigio)
    const a = d.agent || {};
  ...
  window.PDFEngine = { generateFromDOM, generate };
})();

...
(function (G) {
  "use strict";
  if (G.__engineModReady17) return;
  G.__engineModReady17 = true;
  // Reset vecchio guard per forzare reinizializzazione CSS/funzioni
  G.__engineModReady = false;
...
.btn-clear:hover{background:rgba(192,42,42,.12);}`;
  ...
  // Rimuovi CSS vecchio se presente (da versioni precedenti dell'engine)
  const _oldCss = document.getElementById("__emCSS");
  if (_oldCss) _oldCss.remove();

  // Rimuovi overlay vecchio se presente
  const _oldOv = document.getElementById("__emOv");
